...
const customHeader = `
<header id="navbar">
   <a style="margin-right: auto; margin-top: 2px" href="/">

      <div style="padding:0;height:100%">
         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="-16 4 80 16" height="2rem">
            <circle cx="-12" cy="12" r="10" fill="#2e9dfe" />
            
            <g transform="translate(2, 4) scale(0.016, 0.016)">
               <path fill="var(--text)"
...
//    <div id="tsd-search-status" aria-live="polite" aria-atomic="true"></div>
// </dialog>
// <a href="#" class="tsd-widget menu" id="tsd-toolbar-menu-trigger" data-toggle="menu" aria-label="Menu" data-has-instance="true">
...
function walk(dir) {
   for (const name of readdirSync(dir)) {
      ...
      if (statSync(p).isDirectory()) {
         console.log("going deeper in " + p);
         walk(p);
      } else if (p.endsWith(".html")) patchFile(p);
   }
}
...
function patchFile(file) {
   ...
   const cleaned = html
      // .replace(/<header class="tsd-page-toolbar">[\s\S]*?<\/header>/, customHeader)
      .replace(/(<header class="tsd-page-toolbar"[^>]*>)/, `$1${customHeader}`)
      .replace(/<footer[\s\S]*?<\/footer>/, customFooter)

      // replace unneeded elements
      .replace(/<div class="tsd-theme-toggle">[\s\S]*?<\/div>/, "")
      // .replace(/<div class="page-menu">[\s\S]*?<\/div>/, "")

      // remove the element with class: "tsd-accordion"
      .replace(/<details class="tsd-accordion">[\s\S]*?<\/details>/g, "")

      // remove the ul element with aria-label "Breadcrumb"
      .replace(/<ul\b[^>]*class="[^"]*\btsd-breadcrumb\b[^"]*"[^>]*>[\s\S]*?<\/ul>/, "");

   writeFileSync(file, cleaned, "utf8");
}

if (!statSync(outDir, { throwIfNoEntry: false })?.isDirectory()) {
   throw new Error(`TypeDoc output directory not found: ${outDir}`);
}

walk(outDir);
console.log(`Patched TypeDoc navbar HTML in: ${outDir}`);
